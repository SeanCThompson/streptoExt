...
for pos in range(maxlen):
    ...
    # genome_logo[-1] = [x/float(tot_thispos) for x in genome_logo[-1]] # Transforms counts into ratios
        ...
        for i,x in enumerate(genome):
            if x == 'F': 
                try:
                    ...
                    l_gnmsize_posF[ int(i) , len_genome ] += 1
                except:
                    print("Error?")
        ...
        for i,x in enumerate(genome):
            if x == 'A': 
                try:
                    ...
                    l_gnmsize_posA[ int(i) , len_genome ] += 1
                except:
                    print("Error?")
                    print(int(bin_gnm*(i/(flen_genome))) )
                    print(len_genome)
            ...
            for i,x in enumerate(genome):
                if x == 'B': 
                    try:
                        ...
                        l_gnmsize_posB[ int(i) , len_genome ] += 1
            ...
            for i,x in enumerate(genome):
                if x == 'H': 
                    try:
                        ...
                        l_gnmsize_posH[ int(i) , len_genome ] += 1
                    except:
                        print("Error?")
                        print(int(bin_gnm*(i/(flen_genome))) )
                        print(len_genome)
...
min_all_array = np.min( [np.min(l_gnmsize_posF), np.min(l_gnmsize_posA)] )
max_all_array = np.max( [np.max(l_gnmsize_posF), np.max(l_gnmsize_posA)] )

if tot_letter_used==3:
    min_all_array = np.min([min_all_array, np.min(l_gnmsize_posB)])
    max_all_array = np.max([max_all_array, np.max(l_gnmsize_posB)])
...
ax10 = fig.add_subplot(spec2[2:5 , 0:3])
ax00 = fig.add_subplot(spec2[0   , 0:3], sharex=ax10)
ax20 = fig.add_subplot(spec2[1   , 0:3])
# ax_cbarF = fig.add_subplot(spec2[1:4, -1])

ax11 = fig.add_subplot(spec2[2:5 , 3:6], sharey = ax10)
ax01 = fig.add_subplot(spec2[0   , 3:6], sharex = ax11, sharey = ax00) 
ax21 = fig.add_subplot(spec2[1   , 3:6])
...
ax1last = fig.add_subplot(spec2[2:5 , -1], sharey = ax10)
